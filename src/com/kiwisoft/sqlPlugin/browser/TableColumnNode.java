 ...
 */
public class TableColumnNode extends DynamicTreeNode
{
     ...
     */
    public TableColumnNode(Project project, DatabaseColumn column, DefaultTreeModel aTreeModel)
    {
        super(column, aTreeModel);
    ...
    {
        return (DatabaseColumn) getUserObject();
    }
    ...
    {
		DatabaseColumn column=getColumn();
		try
		{
			if (column.getTable().getPrimaryKeys(project).contains(column.getColumnName()))
				return IconManager.getIcon(Icons.PRIMARY_KEY);
		}
		catch (Exception e)
		{
			e.printStackTrace();
		}
		return IconManager.getIcon(Icons.COLUMN);
	}
}
